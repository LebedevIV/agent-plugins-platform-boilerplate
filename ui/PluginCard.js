...
export function createPluginCard(plugin) {
    ...
    // --- ИЗМЕНЕНИЕ ЛОГИКИ КЛИКА ---
    card.onclick = async () => {
        if (card.classList.contains('running')) return;

        console.log(`--- Запуск плагина ${plugin.name} через WorkflowEngine ---`);
        card.classList.add('running');
        icon.src = loaderIconSrc;
        
        try {
            // Вызываем наш новый движок, передавая ему ID плагина
            await runWorkflow(plugin.id);
            console.log(`--- Плагин ${plugin.name} успешно завершил работу ---`);
            // alert(`Плагин ${plugin.name} успешно выполнен! Результат в консоли (F12).`);
        } catch (error) {
            console.error(`--- ОШИБКА во время выполнения плагина ${plugin.name}:`, error);
