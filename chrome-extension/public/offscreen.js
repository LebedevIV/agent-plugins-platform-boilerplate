...
async function executeWorkflowWithChunks(pluginId, pageKey, workflowPayload, requestId, pluginSettings, sendResponse) {
  ...
  try {
    ...
    const analyzeOzonProduct = pyodide.globals.get('analyze_ozon_product');
      ...
      try {
        ...
        try {
          ...
          if (verifyPluginSettings) {
            const keys = Object.keys(verifyPluginSettings.toJs ? verifyPluginSettings.toJs() : verifyPluginSettings);
            logInfo('PYODIDE', `✅ pluginSettings verified: ${keys.length} keys available: ${keys.join(', ')}`);
    ...
    if (workflowPayload.page_html && typeof workflowPayload.page_html === 'object' && workflowPayload.page_html.__isChunkedString) {
      ...
      // HTML данные будут переданы как параметры функции
      logInfo('PYODIDE', 'HTML data will be passed as function parameters');
      ...
      if (workflowPayload.page_html && typeof workflowPayload.page_html === 'string') {
        logInfo('PYODIDE', '✅ Direct HTML data ready for parameter passing');
      } else {
    ...
    // ДОПОЛНИТЕЛЬНОЕ ЛОГИРОВАНИЕ ПОРЯДКА УСТАНОВКИ ПЕРЕМЕННЫХ
    logInfo('PYODIDE', '🎯 FINAL VARIABLE SETUP ORDER VERIFICATION:');
    logInfo('PYODIDE', '✅ Step 1: pluginSettings - SET FIRST');
    logInfo('PYODIDE', '✅ Step 2: HTML data (chunks/metadata) - SET');
    logInfo('PYODIDE', '✅ Step 3: input_data - READY');
    logInfo('PYODIDE', '🚀 Step 4: CALLING PYTHON FUNCTION NOW');

    // ФИНАЛЬНАЯ ПРОВЕРКА ДОСТУПНОСТИ ВСЕХ ПЕРЕМЕННЫХ ПЕРЕД ВЫЗОВОМ
    try {
      const finalCheck = {
        pluginSettings: pyodide.globals['pluginSettings'] ? 'AVAILABLE' : 'MISSING',
        page_html_chunk_count: pyodide.globals['page_html_chunk_count'] ? 'AVAILABLE' : 'MISSING',
        page_html_total_length: pyodide.globals['page_html_total_length'] ? 'AVAILABLE' : 'MISSING',
        input_data: pyodide.globals['input_data'] ? 'AVAILABLE' : 'MISSING'
      };
      logInfo('PYODIDE', `🎯 FINAL CHECK: ${JSON.stringify(finalCheck)}`);

      // КРИТИЧЕСКАЯ ДИАГНОСТИКА: Проверить ЗНАЧЕНИЯ переменных, а не только наличие
      if (pyodide.globals['page_html_chunk_count']) {
        const chunkCountValue = pyodide.globals['page_html_chunk_count'];
        logInfo('PYODIDE', `🔍 page_html_chunk_count ЗНАЧЕНИЕ: ${chunkCountValue} (тип: ${typeof chunkCountValue})`);
      }
      if (pyodide.globals['page_html_total_length']) {
        const totalLengthValue = pyodide.globals['page_html_total_length'];
        logInfo('PYODIDE', `🔍 page_html_total_length ЗНАЧЕНИЕ: ${totalLengthValue} (тип: ${typeof totalLengthValue})`);
      }
      if (pyodide.globals['page_html_chunk_0']) {
        const chunk0Length = pyodide.globals['page_html_chunk_0'] ? pyodide.globals['page_html_chunk_0'].length : 'NULL';
        logInfo('PYODIDE', `🔍 page_html_chunk_0 ДЛИНА: ${chunk0Length} символов`);
      }

      // Проверить все ключи в globals
      try {
        const allKeys = Array.from(pyodide.globals.keys());
        logInfo('PYODIDE', `📋 Все ключи в pyodide.globals перед вызовом: ${allKeys.join(', ')}`);
      } catch (keysError) {
        logError('PYODIDE', '❌ Не удалось получить ключи globals:', keysError);
      }

    } catch (checkError) {
      logError('PYODIDE', '❌ Error during final variable check:', checkError);
    }

    logInfo('PYODIDE', 'Calling Python function analyze_ozon_product()');

    // Проверка доступности Python функций перед вызовом
    if (!analyzeOzonProduct) {
      console.error("[PYODIDE_DEBUG] analyze_ozon_product function not available");
    ...
    try {
      const criticalCheck = {
        hasAnalyzeOzonProduct: !!analyzeOzonProduct,
        hasInputData: !!pyodide.globals.get('input_data'),
        inputDataType: typeof pyodide.globals.get('input_data'),
        chunkCount: pyodide.globals.get('page_html_chunk_count'),
        totalLength: pyodide.globals.get('page_html_total_length'),
        chunk0Length: pyodide.globals.get('page_html_chunk_0') ? pyodide.globals.get('page_html_chunk_0').length : 'NULL'
      };
      logInfo('PYODIDE', `🚨 CRITICAL PRE-CALL CHECK: ${JSON.stringify(criticalCheck)}`);

      // Проверим все ключи еще раз
      const finalKeys = Array.from(pyodide.globals.keys());
      logInfo('PYODIDE', `🚨 Final globals keys before Python call: ${finalKeys.join(', ')}`);
    ...
    try {
      ...
      resultProxy = await analyzeOzonProduct.callPromising(
        ...
        1, // chunk_count для direct режима
    ...
    console.error("[PYODIDE_LOAD] ❌ Failed to load Python script:", error);
