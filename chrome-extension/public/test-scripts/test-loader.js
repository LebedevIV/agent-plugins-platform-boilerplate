...
// Проверяем, не объявлен ли уже TestLoader
if (typeof TestLoader === 'undefined') {
  class TestLoader {
    constructor() {
  ...
  // Создаем глобальный экземпляр загрузчика только если его еще нет
  if (!window.testLoader) {
    ...
    console.log('🎯 TestLoader уже существует');
  }
} else {
  console.log('🎯 TestLoader уже объявлен');
}
